...
3. An `InfisicalIdentity` waits for its project, then creates or adopts a machine identity and manages its mutable metadata.
...
Creation is explicit through `creationPolicy`, while deletion is safe by default through `deletionPolicy: Orphan`. Setting `Delete` opts into irreversible external deletion. The operator never copies external access tokens into status or generated Secrets.

## Non-goals for the first version

- Synchronizing secrets into workloads; the official Infisical Kubernetes operator already owns that integration.
- Managing organizations, folders, secrets, dynamic secrets, RBAC groups, or identity role assignments.
