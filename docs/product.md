...
5. An `InfisicalIdentity` waits for its project, then creates or adopts a machine identity and manages its mutable metadata and optional permanent project roles.
...
- Managing organizations, folders, secrets, dynamic secrets, RBAC groups, temporary project-role assignments, organization-level identity role assignments, or authentication methods other than Kubernetes Auth.
