...
The connection and identity references are immutable. The configured Kubernetes API URL, network path, CA, reviewer token, and Infisical plan must all permit the selected token-review mode. See the [generated schema](../reference/api.md#infisicalkubernetesauth) and [local operations guide](../../development/operations/local-kind.md).
