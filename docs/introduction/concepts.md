...
Connection
    ↓
Project ──→ Environment
    └────→ Identity ──→ KubernetesAuth / UniversalAuth
```
...
`InfisicalUniversalAuth` can publish a tenant identity’s client credentials to a Secret. A connection can exchange those credentials for short-lived access tokens. `InfisicalKubernetesAuth` configures direct API-server token review for an identity.

The supported project types are `secret-manager` and `cert-manager`. Built-in organization and project roles are supported; custom roles and paid product features are intentionally outside the API.
