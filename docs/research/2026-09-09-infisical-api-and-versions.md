...
The public entity endpoints use `/api/v1/projects`, `/api/v1/projects/{projectId}/environments`, `/api/v1/projects/{projectId}/roles`, `/api/v1/projects/{projectId}/identities`, `/api/v1/identities`, `/api/v1/identities/{identityId}`, `/api/v1/projects/{projectId}/memberships/identities/{identityId}`, and `/api/v1/auth/kubernetes-auth/identities/{identityId}`. Organization lifecycle uses `/api/v2/organizations`, `/api/v1/organization`, `/api/v2/organizations/{organizationId}/memberships`, and `/api/v2/organizations/{organizationId}/workspaces`; the organization membership endpoint is the strongest machine-token-compatible boundary check, while a non-empty workspace response and project visibility provide compatibility fallbacks. The checked-in client deliberately models only the fields and response envelopes needed by the CRDs. It sends bearer authentication, bounds response error bodies, classifies 404s, and applies a per-client timeout. Project-role action responses are accepted in both the string and array forms currently described by the API. Identity membership role requests use the current `roles` array with `isTemporary: false`; responses normalize built-in roles and custom-role slugs to one comparison key. Organization identity listing uses `orgId` and normalizes Infisical’s organization-membership list envelope to the identity model used by the controller.

## Current OpenAPI comparison

The live [Infisical OpenAPI document](https://app.infisical.com/api/docs/json) was checked again on 2026-09-16. It is an OpenAPI 3.0.3 document with 1,486 paths and an informational version of `0.0.1`; that informational value is not used as a server-version pin. The project creation enum is `secret-manager`, `cert-manager`, `kms`, `secret-scanning`, `pam`, and `agent-vault`; the CRD and generated reference use that exact set. Project creation also exposes `kmsKeyId` and `template`, which map to `InfisicalProject.spec.kmsKeyID` and `spec.templateRef`. The project-template endpoints expose the complete current template payload used here: roles, environments, users, groups, organization identities, and project-managed identities. Kubernetes Auth exposes `templateId`; when present, Infisical sources the host, CA, reviewer JWT, review mode, gateway, and audience from the template. The CRD preserves the independently configurable namespace/name allowlists and access-token settings, validates UUID fields, and reports whether the reviewer JWT is template-sourced. The reviewed operation subset and automated check are documented in the [API compatibility policy](../compatibility.md).

The OpenAPI document also contains many product areas outside this operator’s control-plane scope, including secret synchronization, dynamic secrets, PKI, integrations, app connections, groups, and other authentication methods. Those are intentionally not represented as CRDs yet; adding them should be driven by a concrete lifecycle use case rather than by mirroring every API tag. One relevant adjacent omission is `/api/v1/identity-templates`, which supports reusable Kubernetes, OIDC, and LDAP identity-authentication templates. It is tracked in [BL-007](../backlog.md) together with the design work for Universal Auth client-secret lifecycle and token publication.
...
The current OpenAPI document covers the public project, environment, role, identity, membership, project-template, Kubernetes Auth, and organization-access paths used by the client. It still does not describe the organization lifecycle paths used by this project (`/api/v2/organizations` and the user-oriented `/api/v1/organization` endpoints). Those paths remain covered by their official endpoint pages and the client’s HTTP contract tests. The project does not check the entire upstream document; it records and verifies only the supported subset in the [contract manifest](../../hack/infisical-api-contract.json).
