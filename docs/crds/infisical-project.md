...
`InfisicalProject` manages an Infisical project and is the parent for environments and identities.

The free-tier project types are `secret-manager` and `cert-manager`.
...
The connection reference, organization reference, project type, and default-environment behavior are immutable. The project status records the remote project and organization identifiers plus observed environments, but no secret values.
