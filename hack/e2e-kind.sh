...
KUBERNETES_AUTH_REVIEW_URL=${KUBERNETES_AUTH_REVIEW_URL-}
...
if [[ -n "${KUBERNETES_AUTH_REVIEW_URL}" && "${KUBERNETES_AUTH_VERIFY_TLS}" == true ]]; then
...
if [[ -n "${KUBERNETES_AUTH_REVIEW_URL}" ]]; then
...
elif [[ -n "${KUBERNETES_AUTH_REVIEW_URL}" ]]; then
  echo "Kubernetes Auth live check skipped because the local Infisical API rejects the cluster-local review URL"
else
  echo "Kubernetes Auth live check not requested; use kind-kubernetes-auth-e2e with a reachable review URL"
fi
