...
import (
	...
	// Import Kubernetes client auth plugins so local execution supports exec-based credentials.
	...
	infisicalcontroller "github.com/winrarr/infisical-entity-operator/internal/controller/infisical"
...
func init() {
	...
	utilruntime.Must(infisicalv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	// Metrics options configure the optional controller metrics endpoint.
	...
	if secureMetrics {
		// FilterProvider protects the metrics endpoint with Kubernetes authn/authz.
		metricsServerOptions.FilterProvider = filters.WithAuthenticationAndAuthorization
	}

	// If no certificate is specified, controller-runtime generates a self-signed certificate for metrics.
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "999338cb.infisical-operator.io",
	...
	if err := (&infisicalcontroller.InfisicalConnectionReconciler{
	...
	if err := (&infisicalcontroller.InfisicalUniversalAuthReconciler{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "Failed to create controller", "controller", "InfisicalUniversalAuth")
		os.Exit(1)
	}
